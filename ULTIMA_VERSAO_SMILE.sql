...
--SELECT DISTINCT CLI_FOR INTO ##CLI FROM MOV_CON WHERE DATA>='01/01/2016' and data<='10/31/2018'
--OPTION (MAXDOP 1 )
...
--SET @str_comando = @str_comando + ' FROM NEWAGE.DBO.CLIENTES '
...
SET @str_NomeArquivo2=@WORKDIR+'DADOS.CSV'
SET @str_NomeArquivo3=@WORKDIR+'PAGREC2.CSV'
...
Exec xp_cmdshell 'bcp " SELECT CODIGO ,MOTORISTA ,PLACA ,TRANSP ,NUM_CARGA ,RTRIM(LTRIM(CONVERT(CHAR,PEDAGIO))) AS PEDAGIO ,DATA_GER ,NUMERO ,RTRIM(LTRIM(CONVERT(CHAR,PESO))) AS PESO ,RTRIM(LTRIM(CONVERT(CHAR,FRETE))) AS FRETE ,RTRIM(LTRIM(CONVERT(CHAR,CLIENTE))) AS CLIENTE ,PED,EMPRESA  FROM ##VW_VALE_FRETE "  queryout \\srv-sql-hml\d$\Temp\SMILE_BI\DADOS.CSV -c -t ";" -T'
...
SP_HELPTEXT VW_VALE_FRETE

SELECT TOP 5 * FROM VW_VALE_FRETE





SELECT TOP 5 * FROM INVENT_HEAD

SELECT TOP 5 * FROM MOV_MARGEM