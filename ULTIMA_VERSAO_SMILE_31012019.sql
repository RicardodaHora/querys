...
--EXEC FILES_SMILE_BI_NOVO'07/01/2016','10/31/2018'

sp_helptext FILES_SMILE_BI_NOVO

ALTER PROC FILES_SMILE_BI_NOVO @DATA_INI VARCHAR(10),@DATA_FIM VARCHAR(10)
AS
...
SELECT DISTINCT CLI_FOR INTO ##CLI FROM MOV_CON WHERE DATA>=@DATA_INI AND DATA<=@DATA_FIM
OPTION (MAXDOP 1 )
...
SET @str_comando = @str_comando + ' FROM NEWAGE.DBO.MOV_CUSTO WITH (INDEX(MOV_CUSTO_06)NOLOCK)  WHERE CANCELADO IS NULL AND DATA>='''+@DATA_INI+''' AND DATA<='''+@DATA_FIM+''''
...
SET @str_comando = @str_comando + ' FROM NEWAGE.DBO.PAGREC WHERE DATA_PAGA IS NULL AND DATA_EMIS>='''+@DATA_INI+''' AND DATA_EMIS<='''+@DATA_FIM+''''
EXECUTE SP_EXECUTESQL @str_comando
...
--SET @str_comando = @str_comando + ' FROM NEWAGE.DBO.PAGREC WHERE DATA_EMIS>=''06/28/2018'' and DATA_EMIS<=''06/28/2018''  '
SET @str_comando = @str_comando + ' FROM NEWAGE.DBO.PAGREC WHERE DATA_PAGA>='''+@DATA_INI+''' AND DATA_PAGA<='''+@DATA_FIM+''''
EXECUTE SP_EXECUTESQL @str_comando
...
Exec xp_cmdshell 'bcp " SELECT CODFILIAL ,CODCOLIGADA ,DATA ,INTEGRACHAVE ,CODIGO_BFC ,DESCRICAO ,CONTA ,DESCRICAO_CONTA ,CCUSTO ,DESCRICAO_CENTRO_CUSTO ,RTRIM(LTRIM(CONVERT(CHAR,VALOR))) AS VALOR ,RTRIM(LTRIM(CONVERT(CHAR,VALOR_LANC))) AS VALOR_LANC ,RTRIM(LTRIM(CONVERT(CHAR,COD_CLI))) AS COD_CLI ,RAZAO ,RTRIM(LTRIM(CONVERT(CHAR,LCTREF))) AS LCTREF ,DOCUMENTO  FROM ##VW_PARTIDAS_RM "  queryout \\srv-sql-hml\d$\Temp\SMILE_BI\DADOS.CSV -c -t ";" -T'
...
Exec xp_cmdshell 'bcp " SELECT CODIGO ,MOTORISTA ,PLACA ,TRANSP ,NUM_CARGA ,RTRIM(LTRIM(CONVERT(CHAR,PEDAGIO))) AS PEDAGIO ,DATA_GER ,NUMERO ,RTRIM(LTRIM(CONVERT(CHAR,PESO))) AS PESO ,RTRIM(LTRIM(CONVERT(CHAR,FRETE))) AS FRETE ,RTRIM(LTRIM(CONVERT(CHAR,CLIENTE))) AS CLIENTE ,PED  FROM ##VW_VALE_FRETE "  queryout \\srv-sql-hml\d$\Temp\SMILE_BI\DADOS.CSV -c -t ";" -T'
...
SET @str_comando = @str_comando + ' FROM NEWAGE.DBO.INVENT_HEAD WHERE CHAVE IN (SELECT DISTINCT CHAVE FROM MOV_CON WHERE DATA>='''+@DATA_INI+''' AND DATA<='''+@DATA_FIM+''')'
EXECUTE SP_EXECUTESQL @str_comando
...
SET @str_comando = 'SELECT '''
SELECT @str_comando += ''+(RTRIM(A.NAME)+';')  from sys.columns a inner join sys.types b on a.system_type_id = b.user_type_id
where a.object_id = ( SELECT OBJECT_ID FROM SYS.TABLES WHERE NAME = 'INVENT' ) 
...
SET @str_comando = @str_comando + ' FROM NEWAGE.DBO.INVENT	'
EXECUTE SP_EXECUTESQL @str_comando
*/
...
SET @str_comando = +LEFT(@STR_COMANDO,LEN(@STR_COMANDO)-1)
SET @str_comando += ' INTO ##INVENT'
SET @str_comando = @str_comando + ' FROM NEWAGE.DBO.INVENT WHERE CHAVE IN (SELECT DISTINCT CHAVE FROM MOV_CON WHERE DATA>='''+@DATA_INI+''' AND DATA<='''+@DATA_FIM+''')'
SELECT @str_comando
EXECUTE SP_EXECUTESQL @str_comando
